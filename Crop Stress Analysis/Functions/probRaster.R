...
probRaster <- function(list.tmax, list.tmin, begin.month, begin.day, end.month, end.day, 
  ...
  # output.ras1 <-  data.frame( ID = 1:length(output.list[[1]]$probCDF), output.list[[1]]$probCDF)
  # output.ras2 <-  data.frame( ID = 1:length(output.list[[2]]$probCDF), output.list[[2]]$probCDF)
  # 
  # 
  # output.ras <- merge(output.ras1, output.ras2, by = 'ID', all=TRUE)
  # output.ras[is.na(output.ras)] <- 0
  # output.ras = subset(output.ras, select = -c(ID) )
  
  #print("Ready")
  #print(length(output.list[[2]]$probCDF))
