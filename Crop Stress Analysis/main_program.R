# The script is a collaborative development of M. Alfi Hasan, PhD. & Asif Al Faisal

setwd('C:/Users/A.FAISAL/Documents/CIMMYT Scripts/HossainSir/201102_Crop Stress Analysis')     ## Change your working directory here
...
# list.tmax <- splitFun(DF.Tmax.BMD.final[1:length(DF.Tmax.BMD.final[,1]),], 50) ### User input of begain position of the desired dataframe
# 
# list.tmin <- splitFun(DF.Tmin.BMD.final[1:length(DF.Tmin.BMD.final[,1]),], 50) ### User input of begain position of the desired dataframe
# 
...
crop_stage <- " Boro Rice (early) [Seedling Stage]"
...
# list.var <- probRaster(list.tmax, list.tmin, begin.month, begin.day, end.month, end.day, varThreshold.max, varThreshold.min,
#                        raster_grid = dummy.ps, location_df = varlocation ,force.choice = 'Upper')



list.var <- probRaster(list.tmax, list.tmin, begin.month, begin.day, end.month, end.day, varThreshold.max, varThreshold.min,
                            raster_grid = dummy.ps, location_df = varlocation ,force.choice = choice)


list.sel <- cbind(list.var[[1]]$values, list.var[[2]]$values )
...
upVal<- if (roundUpto10(max(list.sel, na.rm = T)) > 1) 1 else roundUpto10(max(list.sel, na.rm = T))
# 
# 
# breaks <- seq(0, 1, by=0.1)*100
...
###############################################################
bd_border <- readOGR('Border/bgd_admbnda_adm0_bbs_20180410.shp')  ### User input of shape files ( for Bangladesh boundary ) ###
river_shp <- readOGR('River Polygon/bgd_hyd_major_rivers_poly_wgs84.shp') ### User input of grid data ( for Bangladesh rivers ) ###
dist_border <- readOGR("bd_bnd_district_line/gadm36_BGD_2_line.shp")
mask_file <- readOGR(file.choose()) ## Select your maskfile
###############################################################


#file.output.name <- paste0(month.abb[begin.month],",",begin.day,"-", month.abb[end.month],",",end.day,".pdf") #  Need to be provided by the user. 
file.output.name <- paste0(month.abb[begin.month],",",begin.day,"-", 
                           month.abb[end.month],",",end.day,",",start_year,"-",end_year,".pdf")

cairo_pdf(file.output.name, width = 6.3, height = 8., family = "serif")

values(dummy.ps) <- list.var[[2]]$values * 100
...
if (choice=='Upper'){
  title <- paste0(crop_stage,"          " ,"Max. Temp.",
                  "\n      ",month.abb[begin.month]," ",begin.day,"-",month.abb[end.month]," ",end.day, 
                  " (",start_year,"-",end_year,")",
                  "           " ,
                  "Threshold: ",varThreshold.max)
}else if (choice=='Lower'){
  title <- paste0(crop_stage,"          " ,"Min. Temp.",
                  "\n      ",month.abb[begin.month]," ",begin.day,"-",month.abb[end.month]," ",end.day,
                  " (",start_year,"-",end_year,")",
                  "           " ,
                  "Threshold: ",varThreshold.min)
}


plot(mask(dummy.ps,mask_file),  col=funcx(12), main=paste0(title),
     breaks=breaks,
